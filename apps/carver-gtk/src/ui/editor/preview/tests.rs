...
#[test]
fn rendered_diff_fallback_should_wrap_every_line_without_embedded_newlines() {
    let html = rendered_document("{.diff}\n```text\n context\n+added\n-removed\n```", false);

    // The injected highlighter re-renders these blocks; the native fallback must
    // match its block-span shape so a scriptless preview keeps one line per row.
    assert!(html.contains("<span class=\"carver-diff-line\"> context</span>"));
    assert!(html.contains("<span class=\"carver-diff-line carver-diff-add\">+added</span>"));
    assert!(html.contains("<span class=\"carver-diff-line carver-diff-remove\">-removed</span>"));
}
